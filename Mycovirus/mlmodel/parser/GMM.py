...
def get_predictions(path, k_min, k_max, num_class, cov_type, seed):
    ...
    plot_div = plotly_dash_show_plot(kmer_table, predictions, "Unsupervised Gaussian Mixture Model")
...
def get_predictions_semi(path,k_min,k_max,num_class,cov_type,seed,labels):
    ...
    gmm.fit(kmer_table)
    predictions = gmm.predict(kmer_table)
    plot_div = plotly_dash_show_plot(kmer_table, predictions, "Semi-Supervised Gaussian Mixture Model")
    output_df.insert(0, "Labels", predictions)
    return [[output_df], [plot_div]]
