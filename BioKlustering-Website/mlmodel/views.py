...
class ResultView(FormView):
    ...
    def process(request):
        ...
        if request.method == 'POST':
            ...
            # send by email
            if sendbyemail and email and len(email) > 0 : # send the result when checkbox is checked and email addr is entered
    ...
    def create_zip(request, duration):
        ...
        param_file.write("note: if you see the parameter values here are different from what you chose, it is because the underlying model finds the best parameter values with the best accuracy based on your input.")
        param_file.close()
